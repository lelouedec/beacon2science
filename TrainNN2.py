...
class Model:
    def __init__(self, local_rank=-1):
        ...
        self.blur = filters.MedianBlur((5,5))
    ...
    def update(self, img0, img1, gt, learning_rate=0, mul=1, training=True, flow_gt=None,timestep=0.5):
        ...
        loss_G    = loss_l1 + loss_tea + loss_distill * 0.0001 
...
import sys
import Sequences_dataset_rdifs
import json
...
if __name__ == "__main__":
    ...
    for i in range(800,1600):
        ...
        for data in dataloader:
            ...
            diff1 = data["diff1"].to(device)
            diff2 = data["diff2"].to(device)
            mid1 = data["mid1"].to(device)
            mid2 = data["mid2"].to(device)
            
      


            model.optimG.zero_grad()
            out,dictio = model.update(diff1, diff2, mid1, learning_rate=1e-5, mul=1, training=True, flow_gt=None,timestep=data["ratio1"].float())
            loss = dictio["bigloss"]
            loss.backward()
            model.optimG.step()

            model.optimG.zero_grad()
            out2,dictio2 = model.update(diff1, diff2, mid2, learning_rate=1e-5, mul=1, training=True, flow_gt=None,timestep=data["ratio2"].float())
            loss = dictio2["bigloss"]
            loss.backward()
            model.optimG.step()
        ...
        with torch.no_grad():
            for data in dataloader_validation:
                ...
                out,dictio = model.update(diff1, diff2, mid1, learning_rate=1e-5, mul=1, training=False, flow_gt=None,timestep=data["ratio1"].float())
                loss = dictio["bigloss"]

                out2,dictio2 = model.update(diff1, diff2, mid2, learning_rate=1e-5, mul=1, training=False, flow_gt=None,timestep=data["ratio2"].float())
                loss = dictio2["bigloss"]

                l1sv.append(dictio["bigloss"].item())
                l2sv.append(dictio2["bigloss"].item())
        ...
        diff1 = diff1[0,0,:,:].cpu().detach().numpy()
        writer.add_image('input/img1', diff1, i, dataformats='HW')
        diff2 =  diff2[0,0,:,:].cpu().detach().numpy()
        writer.add_image('input/img2',diff2 , i, dataformats='HW')

        out =  out[0,0,:,:].cpu().detach().numpy()
        writer.add_image('pred/img1', out, i, dataformats='HW')
        out2 =  out2[0,0,:,:].cpu().detach().numpy()
        writer.add_image('pred/img2', out2, i, dataformats='HW')

        mid1 =  mid1[0,0,:,:].cpu().detach().numpy()
        writer.add_image('GT/img1', mid1, i, dataformats='HW')
        mid2 =  mid2[0,0,:,:].cpu().detach().numpy()
        writer.add_image('GT/img2', mid2, i, dataformats='HW')
        ...
        if(len(losses1_v)>1):
            if(losses1_v[-1]+losses2_v[-1]<best_validation):
                model.save_model(config["model_name2"])
