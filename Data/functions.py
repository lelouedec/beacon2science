...
def ecliptic_cut(data, header, bflag, ftpsc, post_conj, datetime_data, datetime_series, mode):
    ...
    for i in range(len(wcoord)):
        ...
        if mode == 'median':
            ...
            data_mask = np.where(np.logical_and((pa_reg > min(e_val)) , (pa_reg < max(e_val))), data[i], np.nan)

            data_med = np.nanmedian(data_mask, 0)

            dif_cut[arr_ind] = data_med

            elon_mask = np.where(np.logical_and((pa_reg > min(e_val)) , (pa_reg < max(e_val))), elon_reg, np.nan)
            ...
            if i == 0:
                ...
                dif_cut = np.zeros((xsize,date_steps, width_cut))
                el_cut  = np.zeros((xsize,date_steps, width_cut))
                dif_cut[:] = np.nan
                el_cut[:] = np.nan
                arr_ind = 0
...
def hi_prep(im, hdr, post_conj, calpath, pointpath, calibrate_on=True,datatype="science", smask_on=False, fill_mean=True, fill_value=None, update_hdr_on=True, silent=True, **kw_args):
    """
...
def reduction(start,hdul,hdul_data,hdul_header,ftpsc,ins,bflag,calpath,pointpath,silent=False):
     ...
     ## CHANGE rectify inserted here
        ...
        base = 34
        misslist_str = []
        for i in range(len(hdul_header)):
            try:
                lst = hdul_header[i]['MISSLIST']
                misslist_str.append(lst)
            except:
                print(hdul_header[i]['DATE-END'])
                if(i!=0):
                    misslist_str.append(misslist_str[-1])
                else:
                    try:
                        lst = hdul_header[i+1]['MISSLIST']
                        misslist_str.append(lst)
                    except:
                        misslist_str.append(hdul_header[i+2]['MISSLIST'])
